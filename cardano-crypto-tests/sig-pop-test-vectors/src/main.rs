...
use blst::{blst_p1, blst_scalar, blst_sk_to_pk_in_g1, min_sig::*};
...
fn generate_sk_and_pk<R: RngCore>(mut rng: R) -> std::io::Result<()>  {
    ...
    let verif = sig.verify(false, msg, dst, aug, &pk, false);
    ...
    // let result = verify_pairing(&pk, &pop);
